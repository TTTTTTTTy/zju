...
bool inBox(double x0, double y0, double x1, double x2, double y1, double y2);/*判断鼠标是否点击工具条图标*/ 
...
void Main() /*仅初始化执行一次*/
{
    ...
    /*画出工具条*/ 
    DrawGrid(0, 0, WIDTH, HEIGHT, COLUMNS, ROWS);
    MovePen(0.25 * WIDTH, 0.5 * HEIGHT);
    DrawLine(0.5 * WIDTH, 0);
    DrawBox(1.2 * WIDTH, 0.25 * HEIGHT, 0.6 * WIDTH, 0.5 * HEIGHT);
    MovePen(2.9 * WIDTH, 0.5 * HEIGHT);
    DrawEllipticalArc(0.4 * WIDTH, 0.3 * HEIGHT, 0, 360);
    MovePen(3.25 * WIDTH, 0.75 * HEIGHT);
    DrawLine(0.5 * WIDTH, 0);
    MovePen(3.5 * WIDTH, 0.75 * HEIGHT);
    DrawLine(0, -0.5 * HEIGHT);
...
void MouseEventProcess(int x, int y, int button, int event)
{
	...
	switch (event) {
		case BUTTON_DOWN:
			if (inBox(mx, my, 0, WIDTH, 0, HEIGHT)){
				lptr = GetBlock(sizeof(*lptr));
				lptr->x1 = RandomReal(0.0, GetWindowWidth());
				lptr->y1 = RandomReal(0.0, GetWindowHeight());
				lptr->x2 = RandomReal(0.0, GetWindowWidth());
			...
			if(inBox(mx, my, WIDTH, 2 * WIDTH, 0, HEIGHT)){
				rptr = GetBlock(sizeof(*rptr));
			    x1 = RandomReal(0.0, GetWindowWidth());
				x2 = RandomReal(0.0, GetWindowWidth());
				y2 = RandomReal(0.0, GetWindowHeight());
			...
			if(inBox(mx, my, 2 * WIDTH, 3 * WIDTH, 0, HEIGHT)){
				eptr = GetBlock(sizeof(*eptr));
				eptr->cx = RandomReal(1.0, GetWindowWidth()-1.0);
				eptr->cy = RandomReal(1.0, GetWindowHeight()-1.0);
				eptr->rx = RandomReal(0.5, 3.0);
			...
			if(inBox(mx, my, 3 * WIDTH, 4 * WIDTH, 0, HEIGHT)){
				if (inText) break;/*若已处于文本编辑状态，则不生成新的空文本*/
				inText = TRUE; /*进入新的文本行输入状态*/
				tptr = GetBlock(sizeof(*tptr));
				tptr->x = RandomReal(1.0, GetWindowWidth()-1.0);
